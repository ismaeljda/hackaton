...
from services import (
    ...
    AccommodationService, GoogleHotelsService,
    GooglePlacesService
)
...
def destination_page(airport_code):
    ...
    # Import the IATA to city mapping from hotel service
    from services.hotel_service import IATA_TO_CITY
    destination_name = IATA_TO_CITY.get(airport_code, airport_info.get('name', airport_code))
    
    return render_template('destination.html', 
                         destination_code=airport_code,  # Changed from airport_code
                         destination_name=destination_name,  # Added destination name
...
@app.route('/api/restaurants/<destination>')
def get_restaurants(destination):
    try:
        ...
        restaurants = google_places_service.get_restaurants_for_destination(
            destination, 
...
def search_hotels():
    ...
    try:
        ...
        # Convert IATA code to city name for logging and debugging
        from airport_themes import get_airport_info
        airport_info = get_airport_info(destination)
        city_name = airport_info.get('name', destination)
        # Extract clean city name
        if 'Airport' in city_name or 'airport' in city_name:
            city_name = city_name.replace('Airport', '').replace('airport', '').strip()
        print(f"DEBUG: Hotel search for {destination} -> {city_name}")
